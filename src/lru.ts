import LRUCache, { Options as LRUCacheOptions } from 'quick-lru';
import { Collection, type ReadonlyCollection } from '@discordjs/collection';

/**
 * @internal
 */
export interface LRUCollectionConstructor {
    new (options: LRUCacheOptions<NonNullable<unknown>, NonNullable<unknown>>): LRUCollection<
        NonNullable<unknown>,
        NonNullable<unknown>
    >;
    new <K extends NonNullable<unknown>, V extends NonNullable<unknown>>(
        options: LRUCacheOptions<K, V>
    ): LRUCollection<K, V>;

    readonly prototype: LRUCollection<NonNullable<unknown>, NonNullable<unknown>>;
    readonly [Symbol.species]: LRUCollectionConstructor;
}

/**
 * @internal
 */
export interface LRUCollection<K, V> extends LRUCache<K, V> {
    constructor: LRUCollectionConstructor;
}
...
export class LRUCollection<K extends NonNullable<unknown>, V extends NonNullable<unknown>>
    ...
    implements ImplementableCollection<K, V>
{
    /**
    ...
    public random(amount?: number): V | V[] | undefined {
        const array = [...this.values()];
        if (amount === undefined) return array[Math.floor(Math.random() * array.length)];
        if (array.length === 0 || !amount) return [];
        return Array.from(
            { length: Math.min(amount, array.length) },
            (): V => array.splice(Math.floor(Math.random() * array.length), 1)[0]!
        );
    }
    ...
    public randomKey(amount?: number): K | K[] | undefined {
        ...
        return Array.from(
            { length: Math.min(amount, array.length) },
            (): K => array.splice(Math.floor(Math.random() * array.length), 1)[0]!
        );
    }
    ...
    public flatMap<T>(
        ...
        const collections = this.map(fn, thisArgument);
        // eslint-disable-next-line unicorn/prefer-spread
        return new Collection<K, T>().concat(...collections);
    }
    ...
    public mapValues<T>(
        ...
        for (const [key, value] of this) coll.set(key, fn(value, key, this));
        return coll;
    }
    ...
    public concat(...collections: ReadonlyCollection<K, V>[]) {
        ...
        for (const coll of collections) {
            for (const [key, value] of coll) newColl.set(key, value);
        }
    ...
    public sort(compareFunction: Comparator<K, V> = LRUCollection.defaultSort) {
     ...
     */
    // eslint-disable-next-line @typescript-eslint/unbound-method
    public sorted(compareFunction: Comparator<K, V> = LRUCollection.defaultSort) {
        return new Collection(this).sort((av, bv, ak, bk) => compareFunction(av, bv, ak, bk));
    }
